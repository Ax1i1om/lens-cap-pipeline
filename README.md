...
> **ALPHA · v0.1.0-alpha.3**：这是加入可分发质量参考包和干净上下文交接的公开预览版。配置 schema、模型适配器和
...
图像 Skill 的质量参考也随仓库分发，不依赖本次对话或本机旧任务。先验证六张用户精选
参考图，再同步 Skill：

```sh
./scripts/reference_pack.py check
./scripts/reference_pack.py list
./scripts/install_skills.py sync --dest .agents/skills --apply
```

安装器会连同 `skills/lens-cap-imagegen/references/quality-library/` 一起复制，并把
文件哈希写入 `.lens-cap-skills.json`。新建 Codex 任务后，镜头盖图稿 Skill 会默认使用
奥德赛质量参考；月球／轨道／烛光方向会切换到月球参考。用户有自己的海报或艺术参考
图时，Skill 会先询问并以用户上传图优先。参考图只提供完成度、层级、线条节奏和色块
关系，不会覆盖当前镜头的焦段、光圈、品牌文字或历史证据。
