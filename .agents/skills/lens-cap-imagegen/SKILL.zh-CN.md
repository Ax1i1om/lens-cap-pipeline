...
数组，同一张图适用时可以同时记录多个角色。只提取
整合度、层级、边缘纪律、空间节奏和完成度下限等可迁移品质，并为 v2 审核保存本地
快照及哈希，不复制其中镜头专属的文字、故事、版式、母题或装饰密度。用户明确要求
...
为每个选中的锚点记录背景、证据等级、上图角色、视觉职责和唯一稳定 `anchor_id`。把它当作
**主锚点系统**，而不是一个放大的图标：同一个有来源的观念必须跨文字／反形、
场域／分割／路径、容器／圆周节奏中的至少两类，产生两个可观察、功能不同但协调的
结构后果。复制或缩放同一图标不能算两次；一个连续动作若明显同时支配两种功能则可以。
balanced 模式可以让它在视觉重量上低于焦段／光圈，但不能失去主结构权限；
mythic 模式还可让它主导外轮廓。做“隐藏文字测试”时，遮住除焦段和光圈以外
所有承载身份的文字与数字；关联仍应通过几何可感知，但图形不能像官方 Logo、
角色、剧照或产品背书。普通同心圆、径向刻度、网格和仪表标记本身不能通过此测试。
...
锚点、该锚点如何从研究转化为结构、带限定的传闻文案、参考图 `roles` 数组和负面约束。
还必须用主动动词说明因果关系：锚点如何进入、分割、转向、重复、形成反形、咬合
主文字或收束到圆周。并排罗列元素、在未改造的通用字形旁放一个箭头／大图标，或用
任意填充面板补空，都不算构图。数组中的参考图角色可为 `quality_reference`、
`style_reference`、`edit_target` 或 `exact_content_reference`。
